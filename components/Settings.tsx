...
import { Trash2, LogOut, Target, Sliders, ShieldAlert, Moon, Link, Check, Database, Key, Activity, FileCode, Copy, Timer, ExternalLink, HelpCircle, Loader2, AlertCircle, User as UserIcon } from 'lucide-react';
...
export const Settings: React.FC<SettingsProps> = ({ user, onWipe, onUpdateScore, onUpdatePreferences, onUpdateCallsign, onLogout }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-32">
      ...
      {/* Database Connection */}
      <section className={`glass rounded-3xl overflow-hidden transition-colors border-2 ${
          testStatus === 'success' ? 'border-act-green/30' : 
         ...
         <div className="p-8 space-y-8">
            ...
            {testStatus !== 'idle' && (
                <div className={`flex items-center gap-3 p-4 rounded-xl border animate-in fade-in slide-in-from-top-1 ${
                    testStatus === 'success' ? 'bg-act-green/10 border-act-green/20 text-act-green' : 
            ...
            <div className="flex justify-between items-center pt-4">
                ...
                <button 
                    ...
                    className={`px-8 py-3 rounded-xl font-mono text-[10px] font-bold uppercase transition-all flex items-center gap-2 disabled:opacity-50 ${
                        testStatus === 'success' ? 'bg-act-green text-black' : 
